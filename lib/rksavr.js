...
export class RKSAVR {
  ...
  _initOptimizer() {
    ...
    switch (type) {
      ...
      case 'annealing':
        return (fn) => {
          const result = simulatedAnnealing(
              (x) => fn(x[0]),
              [0.5],
              {maxIter: 2000, coolingRate: 0.99, stepSize: 0.1},
          );
          return result.x[0];
        };
      case 'de':
        return (fn) => {
          const result = differentialEvolution(
              (x) => fn(x[0]),
              [0.5],
      ...
      case 'ags':
        return (fn, min, guess) => adaptiveGridSearch(fn, min, guess).x;
      case 'brent':
      default:
        return (fn, min, guess) => brentMinimize(fn, min, guess[0] || 0.5, 1.0, 1e-5).x;
    }
  }
  ...
  estimateSingle(window, opts = {}) {
    ...
    return optimizer(fn, 0.0001, [0.5]);
  }
  ...
  rollingMultiScale(series, windowSize, scales, weights = null, step = 1, onProgress) {
    ...
    const multiConfig = {scales, weights, iterations: 1};

    for (let t = 0; t <= series.length - windowSize; t += step) {
      const window = series.slice(t, t + windowSize);
      const H = this.estimate(window, multiConfig);

      const incMap = RKSAVR.getIncrementsMulti(window, scales);
      const samples = scales.map((scale) => {
        const inc = incMap.get(scale);
        return this.sampler(inc, this.sampleSize).sort((a, b) => a - b);
      });

      const profile = [];
      for (let i = 0; i < samples.length; i++) {
        for (let j = i + 1; j < samples.length; j++) {
          const dist = computeScalePairDistance(samples, scales, i, j, H);
          profile.push(dist);
        }
      }
