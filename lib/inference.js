...
export function detectBreakpoints(hEstimates, opts = {}) {
  ...
  for (let i = 0; i < n; i++) {
    prefixSum[i + 1] = prefixSum[i] + hEstimates[i];
    prefixSumSq[i + 1] = prefixSumSq[i] + hEstimates[i] * hEstimates[i];
  }

  function segmentCost(start, end) {
    const count = end - start;
    if (count === 0) return {cost: 0, mean: 0};
    const sum = prefixSum[end] - prefixSum[start];
    const sumSq = prefixSumSq[end] - prefixSumSq[start];
    const mean = sum / count;
    const variance = (sumSq - sum * mean) / count;
    return {cost: variance * count, mean};
  }
  ...
  while (start < n - minSegLen) {
    ...
    for (let t = start + minSegLen; t < end - minSegLen; t++) {
      const left = segmentCost(start, t);
      const right = segmentCost(t, end);
    ...
    const left = segmentCost(start, bestBreak);
    breakpoints.push({
      start,
      end: bestBreak,
      meanH: left.mean,
  ...
  if (start < n) {
    const tail = segmentCost(start, n);
    breakpoints.push({
      start,
      end: n,
      meanH: tail.mean,
