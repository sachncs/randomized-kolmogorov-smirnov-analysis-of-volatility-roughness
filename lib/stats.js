...
/**
 * Generates batch of standard normal random variables.
 * @param {number} n Number of samples.
 * @return {Float64Array} Array of standard normals.
 */
...
/**
 * Generates correlated Gaussian random variables.
 * @param {number} n Number of samples.
 * @param {number} rho Correlation coefficient (-1 to 1).
 * @return {Array<Float64Array>} [Z1, Z2] correlated standard normals.
 */
export function correlatedGaussian(n, rho) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    Z1[i] = z1;
    Z2[i] = rho * z1 + Math.sqrt(Math.max(0, 1 - rho * rho)) * z2;
  }
