...
export function rBergomi(params = {}) {
  ...
  for (let p = 0; p < nPaths; p++) {
    ...
    // Compute the fractional integral cumulatively
    // I_t = sum_{j=0}^{t-1} K(t-j) * dW_perp_j
    // We can compute this incrementally to avoid O(n^2)
    const I = new Float64Array(nSteps + 1);
    I[0] = 0;

    for (let t = 1; t <= nSteps; t++) {
      // I_t = I_{t-1} + K(1) * dW_perp_{t-1} + sum_{j=0}^{t-2} (K(t-j) - K(t-1-j)) * dW_perp_j
      // But for simplicity we use the direct sum with cumulative update
      let sum = 0;
      for (let j = 0; j < t; j++) {
...
export function rBergomiPrice(params = {}) {
  ...
  for (const volPath of paths) {
    const pricePath = new Float64Array(nSteps + 1);
    pricePath[0] = s0;

    // Generate additional Brownian for price (aligned with vol's W)
    const [dW] = correlatedGaussian(nSteps, -0.8);
